...
function _copy() {
    ...
    xcode_template_path="$HOME/Library/Developer/Xcode/Templates/File Templates/cocos2d-x"
    ...
    rm -rf "$xcode_template_path"
    mkdir -p "$xcode_template_path"

    echo $'\e[34mStart to copy files\e[m'
    ls | grep xctemplate | sed | while read dir; do
        echo "  $dir"
        mkdir -p "$xcode_template_path/$dir"
        cp -r "$script_dir/../$dir" "$xcode_template_path"
    done
